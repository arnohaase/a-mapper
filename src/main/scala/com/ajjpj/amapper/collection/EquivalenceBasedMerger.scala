...
class EquivalenceBasedMerger[SE <: AnyRef, S<:Iterable[SE], T <: AMutableCollection[TE], TE<:AnyRef, H <: ACollectionHelper] {
  def map(source: S, sourceType: AType, sourceQualifier: AQualifier, targetRaw: T, targetType: AType, targetQualifier: AQualifier, worker: AMapperWorker[_ <: H], path: PathBuilder): T = {
    ...
    if(target.asIterable.isEmpty) // this is a performance optimization
      equiv.sourceWithoutTarget.foreach(s => target.add(worker.map(ACollectionSupport.elementPath(path, worker, s, sourceElementType), s, sourceElementType, sourceQualifier, null, targetElementType, targetQualifier).asInstanceOf[TE]))
    else {
      equiv.targetWithoutSource.foreach(target.remove)
      equiv.sourceWithoutTarget.map(s => target.add(worker.map(path + ParameterizedPathSegment("elements", worker.helpers.uniqueIdentifier(s, sourceElementType)), s, sourceElementType, sourceQualifier, null, targetElementType, targetQualifier).asInstanceOf[TE]))
      ...
      equiv.sourcesWithTargetEquivalent.foreach (s => equiv.targetEquivalents(s) match {
        case e if e.size == 1 && e.iterator.next().count == 1 => //TODO unapply for set with a single element
          worker.map(ACollectionSupport.elementPath(path, worker, s, sourceElementType), s, sourceElementType, sourceQualifier, e.iterator.next(), targetElementType, targetQualifier)
        case e => // special case: several existing equivalents in the target collection for the same source element
          ...
          target.add(worker.map(ACollectionSupport.elementPath(path, worker, s, sourceElementType), s, sourceElementType, sourceQualifier, e.iterator.next(), targetElementType, targetQualifier).asInstanceOf[TE])
      })
    }
